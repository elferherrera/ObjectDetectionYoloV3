"""
Layer and definitions for YoloV3
"""
import torch
...
from object_detection_yolo3.models.utils.parse_config import parse_model_config
from object_detection_yolo3.models.utils.utils import build_targets
...
class Darknet(nn.Module):
    ...
    def load_darknet_weights(self, weights_path):
        ...
        with open(weights_path, "rb") as reader:
            header = np.fromfile(reader, dtype=np.int32, count=5)  # First five are header values
            self.header_info = header  # Needed to write header when saving weights
            self.seen = header[3]  # number of images seen during training
            weights = np.fromfile(reader, dtype=np.float32)  # The rest are weights
